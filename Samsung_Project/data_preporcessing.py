...
stop_words2 = ['연합뉴스', 'yna', 'co', 'kr', 'DB', 'DB 금지', 'co.kr', '섹션', '분류', '서울', '(=)', '재판매', '기자', '구독', '금지' , '및', '또', '통해',
              ...
              '오전', '을', '위해', '는', '중', '이', '말했다', '에', '그', '한다', '했다', '를', '도', '해', '대한', 'DN', '것', '들', '로', 'A씨', '따라',
              '관련', '라고', '당시', '전날', '함께', '따르면', '다']
...
# #두글자 이상만 글자 추출
# newsData['내용'] = newsData['내용'].apply(lambda x: ' '.join([word for word in x.split() if len(word) > 1]))

# # 날짜 포함 파일 저장
# newsData[['내용','날짜']].to_csv('./data/wordcloud.csv', encoding='utf-8')
...
okt = Okt()
newsData['명사'] = newsData['내용'].apply(lambda x: ' '.join([word for word in okt.nouns(str(x)) if len(word) > 1])) #두글자 이상만 추출
