...
for day in range(1,32) :
    ...
    for elements in newsList_bs.select('div.list_body.newsflash_body > ul li'):
        ...
        if article_bs.select_one('h2#title_area') != None:
            ...
            contents = article_bs.select_one('article#dic_area').text.strip() #본문내용
            ...
            contents = article_bs.select_one('div#newsEndContents').text.strip() #본문내용
            ...
            article_date = extract_date(article_datetime) #날짜
            content = article_bs.select_one('#articeBody').text.strip() #본문내용
