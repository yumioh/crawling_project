...
for elements in newsList_bs.select('div.list_body.newsflash_body > ul li'):
    ...
    if article_bs.select_one('h2#title_area') != None:
        ...
        contents = article_bs.select_one('article#dic_area').getText().replace("\n", "").strip() #본문내용
        ...
        article_date = extract_date(article_datetime)
        contents = article_bs.select_one('div#newsEndContents').getText().replace("\n", "").strip() #본문내용
        row.append([article_date, title, cleanup_content(contents), '스포츠'])
        news_num = article_bs.select_one('div._reactionModule.u_likeit').attrs['data-cid']
        if news_num :
        ...
        article_date = extract_date(article_datetime)
        content = article_bs.select_one('#articeBody').getText().replace("\n", "").strip() #본문내용
        #print(article_bs.select_one('em.guide_categorization_item')) #카테고리
        row.append([article_date, title, cleanup_content(contents), '연예'])
    else:
