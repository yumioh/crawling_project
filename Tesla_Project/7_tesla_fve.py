...
ax.bar(stocks_df['날짜'], stocks_df['거래량'], label='Telsa Stock', color='#555555')
#ax.set_title('Volume for Topic : X', fontsize=20)
#그래프 제목
ax.set_title(keyword, fontsize=20)

ax.set_xlabel('Date', fontsize=14)
ax.set_ylabel('Stock Volume', fontsize=14)
ax.legend(fontsize=12, loc='best')

#해당하는 토픽 거래량 ex. 테슬라
ax2 = ax.twinx()
ax2.bar(filtered_stock_rows['날짜'], filtered_stock_rows['거래량'], label='Topic', color='red')
ax2.set_ylabel('News Volume', fontsize=14)
...
#그래프 파일 저장
plt.savefig("./Tesla_Project/data/visualization/" + keyword + ".png", format = 'png')
