...
## 2. LDA 모델링
    ...
    - LDA로 뉴스 정보를 주제별로 특징으로 분해하여 다차원 척도법으로 각 토픽들 간의 관계 확인
...
## 5. 결론
    - FVE를 그래프를 보면 일부 키워드는 주가과 상관 관계가 있어 보인다. 다만, 적은 데이터를 기준으로 키워드를 추출하여,
      주가과 연관성이 낮은 키워드들도 다수 보여 뉴스의 키워드와 주식시장의 연관관계를 증빙하는데 부족함.
