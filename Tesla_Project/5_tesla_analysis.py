...
#딕셔너리 생성 : 다시하기 
dic = Dictionary()
clean_words = words_df['pos_content']
print('clean dataframe', clean_words[:10])

#clean_words = words_df['pos_content'].apply(lambda x: str(x).split())
clean_words = clean_words.apply(lambda x: ' '.join(map(str, x)) if isinstance(x, list) else str(x)).apply(lambda x: x.split())
id2word = Dictionary(clean_words)
print("id2word : ", list(id2word.items())[:10])

# 공백을 기준으로 split하여 리스트로 변환
# clean_words = clean_words.apply(lambda x: x.split())
# id2word = Dictionary(clean_words)
